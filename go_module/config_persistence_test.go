...
func awaitCredentialAction(t *testing.T, action func(func(error))) error {
	...
	case <-time.After(5*time.Second): t.Fatal("credential operation did not finish"); return nil
	}
}
...
func TestConfigMigrationRetainsLegacyUntilSecurePublication(t *testing.T) {
	...
	if err := awaitCredentialAction(t, a.loadCredentials); err == nil { t.Fatal("locked store allowed legacy credential removal") }
	got, err := os.ReadFile(path)
	if err != nil || !bytes.Equal(got, original) { t.Fatalf("failed migration changed original: %v", err) }
	store.readErr = nil
	if err := awaitCredentialAction(t, a.loadCredentials); err != nil { t.Fatal(err) }
	got, err = os.ReadFile(path)
	if err != nil { t.Fatal(err) }
	if bytes.Contains(got, []byte("legacy-fixture-secret")) || bytes.Contains(got, []byte("github_token")) { t.Fatal("successful migration persisted plaintext credential") }
...
func TestConfigPublicationFailureRemainsRetryable(t *testing.T) {
	...
	if err := os.WriteFile(originalPath, original, 0600); err != nil { t.Fatal(err) }
	if err := os.Symlink(originalPath, path); err != nil { t.Skipf("symlink unavailable: %v", err) }
	a := &App{configPath: path, legacyTokenPending: "retry-fixture-secret"}
	if err := awaitCredentialAction(t, a.loadCredentials); err == nil { t.Fatal("unsafe destination accepted") }
	got, err := os.ReadFile(originalPath)
	if err != nil || !bytes.Equal(got, original) { t.Fatal("publication failure changed original") }
	...
	if err := os.Rename(originalPath, path); err != nil { t.Fatal(err) }
	if err := a.persistConfig(); err != nil { t.Fatal(err) }
	got, err = os.ReadFile(path)
	if err != nil || bytes.Contains(got, []byte("retry-fixture-secret")) { t.Fatal("retry did not remove migrated plaintext") }
}
...
func TestExplicitCredentialDeletionCannotReviveLegacyOnRetry(t *testing.T) {
	...
	installTestCredentialStore(t, store)
	path := filepath.Join(t.TempDir(), "config.json")
	if err := os.WriteFile(path, []byte(`{"github_token":"obsolete-fixture-secret"}`), 0600); err != nil { t.Fatal(err) }
	a := &App{configPath: path, legacyTokenPending: "obsolete-fixture-secret"}
	if err := awaitCredentialAction(t, func(done func(error)) { a.storeCredential("replacement-fixture-secret", done) }); err != nil { t.Fatal(err) }
	if store.token != "replacement-fixture-secret" {
		t.Fatal("explicit replacement did not supersede the legacy credential")
	}
	if err := awaitCredentialAction(t, func(done func(error)) { a.storeCredential("", done) }); err != nil { t.Fatal(err) }
	retained := path + ".retained"
	if err := os.Rename(path, retained); err != nil { t.Fatal(err) }
	if err := os.Symlink(retained, path); err != nil { t.Skipf("symlink unavailable: %v", err) }
	if err := a.persistConfig(); err == nil { t.Fatal("unsafe publication unexpectedly succeeded") }
	if store.token != "" {
		t.Fatal("failed preferences publication revived a deleted credential")
	}
	if err := os.Remove(path); err != nil { t.Fatal(err) }
	if err := os.Rename(retained, path); err != nil { t.Fatal(err) }
	if err := a.persistConfig(); err != nil { t.Fatal(err) }
	if store.token != "" {
...
func TestPendingCredentialProviderDoesNotBlockLocalEditing(t *testing.T) {
	...
	select {
	case <-returned:
	case <-time.After(2*time.Second): close(release); <-done; t.Fatal("native credential provider blocked the calling UI action")
	}
	entry := NewInputEntry()
	changed := false
	entry.OnChanged = func(string) { changed = true }
	test.Type(entry, "local draft remains editable")
	if !changed || entry.Text != "local draft remains editable" { close(release); <-done; t.Fatal("local editor interaction did not complete while credential provider was pending") }
	if err := awaitCredentialAction(t, a.loadCredentials); err == nil { close(release); <-done; t.Fatal("second provider operation was started while one was pending") }
	close(release)
	if err := <-done; err != nil { t.Fatal(err) }
	if a.config.GitHubToken != "stored-fixture" { t.Fatal("completed credential was not usable by contributions") }
	readNativeCredential = func() (string,error) { return "", errors.New("locked fixture store") }
	if err := awaitCredentialAction(t, a.loadCredentials); err == nil { t.Fatal("native provider failure was hidden") }
	if a.config.GitHubToken != "stored-fixture" { t.Fatal("failed reload discarded a usable in-memory credential") }
}
