...
ip.keys = []string{}
	...
	for _, c := range GetClasses() {
		if filter == "" || strings.Contains(strings.ToLower(c.Name), filter) { ip.keys = append(ip.keys, c.Name) }
	}
	for _, f := range GetClassFlags() {
		if filter == "" || strings.Contains(strings.ToLower(f.Name), filter) { ip.keys = append(ip.keys, f.Name) }
	}
	for _, s := range GetSaberStyles() {
		if filter == "" || strings.Contains(strings.ToLower(s.Name), filter) { ip.keys = append(ip.keys, s.Name) }
	}
	for _, g := range GetGlossary() {
		if filter == "" || strings.Contains(strings.ToLower(g.Name), filter) { ip.keys = append(ip.keys, g.Name) }
	}
	...
	var found bool
		...
		formatRich := func(desc, overview string, tips []string, levels map[string]LevelDoc, stats map[string]string, tags []string) string {
			...
			if !strings.HasPrefix(context, "Level ") && len(levels) > 0 {
				...
				var lvls []int
				for k := range levels {
					if v, err := strconv.Atoi(k); err == nil { lvls = append(lvls, v) }
				}
				sort.Ints(lvls)
				for _, l := range lvls {
					lStr := strconv.Itoa(l)
			...
			if len(tips) > 0 {
				sb.WriteString("### Tips\n")
				for _, t := range tips { sb.WriteString("* " + t + "\n") }
			}
			
			if len(tags) > 0 {
				sb.WriteString("\n---\n")
				for _, t := range tags { sb.WriteString(fmt.Sprintf("`#%s` ", t)) }
				sb.WriteString("\n")
			}
